...
<div class="wrap">
  ...
  <div class="role main">
      ...
      <div class="option">
        <button class="btn" @click="create('panel')">创建会员</button>
      </div>
  ...
  methods: {
    ...
    create (name) {
      this.types = [
        ...
        {name: 'save', text: '保存', url: 'member_insert'}
      ]
      console.log(name)
      this.resetsingle()
    ...
    edit (idx) {
      this.types = [
          {name: 'quit', text: '退出', url: ''},
          {name: 'save', text: '保存', url: 'member_update'}
      ]
    ...
    del (idx, id) {
      ...
      this.$refs.dialog.confirm().then(() => {
        api.post({JSON: JSON.stringify(updata)}, 'member_delete').then((item) => {
          this.getdata()
        })
      })
    },
